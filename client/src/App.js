...
class App extends Component {
  ...
  componentDidMount = async () => {
    try {
      ...
      const deployedNetwork = SupplyChainContract.networks[5777];
      const instance = await new web3.eth.Contract(
        SupplyChainContract.abi,
        deployedNetwork && deployedNetwork.address,
      );
      console.log("Kaustubh:kkk"+instance);
  ...
  runExample = async () => {
    const { contract } = this.state;//Kaustubh:Edit
    console.log("Kaustubh:"+contract);
  };
